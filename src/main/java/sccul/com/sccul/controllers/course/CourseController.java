...
public class CourseController {
    ...
    @PutMapping("/{id:[0-9]+}")
    public ResponseEntity<CustomResponse<Course>> update(@Valid @PathVariable long id, @RequestBody CourseDto course) {
        course.setId(id);
        return new ResponseEntity<>(this.service.update(course.castToCourse()), HttpStatus.OK);
    }

    @PatchMapping("/{id:[0-9]+}")
    public ResponseEntity<CustomResponse<Integer>> changeDiscount(@Valid @PathVariable Long id, @RequestBody CourseDto course) {
        course.setId(id);
